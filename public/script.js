...
function waitForLibrariesAndInit() {
    const checkInterval = setInterval(() => {
        // Korean: 필요한 모든 라이브러리가 로드되었는지 확인합니다.
        // English: Check if all required libraries are loaded.
...
async function initializeApp() {
    // Korean: 라이브러리가 로드된 후 변수들을 초기화합니다.
    // English: Initialize variables after libraries are loaded.
    httpProvider = new TonWeb.HttpProvider('https://toncenter.com/api/v2/jsonRPC');
    tonweb = new TonWeb(httpProvider);
