...
- **(KO) 지갑 재연결 및 `Invalid CRC32C` 오류 등 모든 스핀 관련 오류 근본 해결:**
  ...
  - **원인 (Cause):** 1) **지갑 재연결 실패:** `tonconnect-manifest.json`의 `url`이 로컬 주소(`localhost`)로 되어 있어, 실제 서비스 환경(`https://aiandyou.me`)과 불일치. 2) **`Invalid CRC32C` 오류:** Jetton 전송 정보에 포함된 불필요한 `forward_payload` 및 `forward_ton_amount` 필드가 데이터 구조를 손상시킴. 3) **API 오류:** `Toncenter` API의 `runMethod` 기능이 유료 플랜에서만 제공되어 발생.
  - **해결 (Solution):**
    1.  **지갑 연결 문제 해결:** `tonconnect-manifest.json`의 `url`을 실제 서비스 주소인 `https://aiandyou.me`로 수정하여 재연결 문제를 해결했습니다.
...
- **(EN) Fundamentally Resolved All Spin-Related Errors, Including Wallet Reconnection and `Invalid CRC32C`:**
  ...
  - **Cause:** 1) **Wallet Reconnection Failure:** The `url` in `tonconnect-manifest.json` was set to a local address (`localhost`), mismatching the production environment (`https://aiandyou.me`). 2) **`Invalid CRC32C` Error:** Unnecessary `forward_payload` and `forward_ton_amount` fields in the Jetton transfer message were corrupting the data structure. 3) **API Errors:** The `runMethod` feature of the `Toncenter` API is only available on paid plans.
  - **Solution:**
    1.  **Fixed Wallet Connection:** Resolved the reconnection issue by correcting the `url` in `tonconnect-manifest.json` to the production service address.
