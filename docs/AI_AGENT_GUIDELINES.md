...
1.  **진단 (Diagnose): `reports` 작성**

      - (KO) 코드 변경에 앞서, 먼저 `docs/reports/` 폴더에 **코드 진단 및 개선 계획 보고서**를 작성하여 문제점, 근거, 해결 전략을 제안합니다.
      - (EN) Before changing code, first create a **Code Diagnostics and Improvement Plan** report in the `docs/reports/` folder to propose the problem, rationale, and solution strategy.

2.  **결정 (Decide): `adr` 작성 및 문서 업데이트**

      - (KO) 제안된 전략이 주요 아키텍처 변경을 수반할 경우, `docs/adr/` 폴더에 해당 결정에 대한 **ADR(아키텍처 결정 기록) 문서**를 작성하여 사용자 승인을 받습니다.
      - (EN) If the proposed strategy involves significant architectural changes, create an **Architecture Decision Record (ADR)** in the `docs/adr/` folder detailing the context, decision, and consequences, and obtain user approval.
      - (KO) 이 단계에서 `PROJECT_REQUIREMENTS.md`와 `PROJECT_ARCHITECTURE.MD` 등 관련된 핵심 문서들도 함께 수정하여 제출합니다.
      - (EN) At this stage, submit updates to core related documents such as `PROJECT_REQUIREMENTS.md` and `PROJECT_ARCHITECTURE.MD` as well.

3.  **실행 (Execute): 코드 구현**

      - (KO) 승인된 보고서와 ADR, 그리고 수정된 핵심 문서를 바탕으로 실제 코드 구현을 진행합니다.
      - (EN) Based on the approved reports, ADRs, and updated core documents, proceed with the actual code implementation.

4.  **기록 (Log): `CHANGELOG.md` 업데이트 및 PR 제출**

      - (KO) 모든 코드 변경 사항과 함께 `docs/CHANGELOG.md`에 최종 변경 이력을 기록하고 하나의 Pull Request로 제출합니다.
      - (EN) Log the final changes in `docs/CHANGELOG.md` along with all code modifications and submit them as a single Pull Request.
