...
export async function onRequestPost(context) {
  try {
    ...
    if (!winTicket || !choice || !['red', 'black'].includes(choice)) {
      return new Response(JSON.stringify({ success: false, errorCode: "INVALID_REQUEST", message: "winTicket and choice ('red' or 'black') are required." }), { status: 400 });
    }
    ...
    if (!ticketId) {
        return new Response(JSON.stringify({ success: false, errorCode: "INVALID_TICKET_PAYLOAD", message: "Ticket payload is missing 'spinId'." }), { status: 400 });
    }
    ...
    if (doubleUpCount > MAX_DOUBLE_UP) {
        return new Response(JSON.stringify({ success: false, errorCode: "MAX_DOUBLE_UP_REACHED", message: "Maximum double-up limit reached." }), { status: 403 });
    }
    ...
    if (isWin) {
      ...
      const newTicket = await new SignJWT({ ...payload, payout: newPayout, doubleUpCount, spinId: crypto.randomUUID() }) // (KO) 새로운 spinId 생성 (EN) Generate new spinId
