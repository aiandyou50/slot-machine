...
async function verifyTransaction(boc, betAmount) {
  try {
    ...
    if (op !== 0x0f8a7ea5) {
      throw new Error(
        `Invalid operation code. Expected 0x0f8a7ea5, got ${op.toString(16)}`
      );
    }
    ...
    if (jettonAmount !== betAmount.toString()) {
      throw new Error(
        `Bet amount mismatch. Expected ${betAmount}, got ${jettonAmount}`
      );
    }

    if (
      toAddress.toString() !== Address.parse(GAME_WALLET_ADDRESS).toString()
    ) {
...
export async function onRequestPost(context) {
  try {
    ...
    if (!boc || !betAmount) {
      return new Response(
        JSON.stringify({
          ...
          message: 'boc and betAmount are required.',
        }),
        { status: 400 }
      );
    }
    ...
    if (!isDevMode) {
      const { isValid, error, senderAddress } = await verifyTransaction(
        boc,
        betAmount
      );
      if (!isValid) {
        return new Response(
          JSON.stringify({
            ...
            message: `BOC verification failed: ${error}`,
          }),
          { status: 400 }
        );
      }
      ...
      if (!userAddress) {
        return new Response(
          JSON.stringify({
            success: false,
            errorCode: 'INVALID_REQUEST',
            message: 'userAddress is required for dev mode.',
          }),
          { status: 400 }
        );
      }
    ...
    if (win) {
      const winTicket = await new SignJWT({
        payout,
      ...
      return new Response(
        JSON.stringify({ success: true, reels, win, payout, winTicket }),
        {
      ...
      return new Response(
        JSON.stringify({ success: true, reels, win, payout }),
        {
          headers: { 'Content-Type': 'application/json' },
        }
      );
