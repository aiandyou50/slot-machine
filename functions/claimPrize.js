...
async function sendTransaction(userAddress, amount, env) {
  if (!env.GAME_WALLET_SEED) {
    throw new Error(
      'CRITICAL: GAME_WALLET_SEED is not configured in environment variables.'
    );
  }
  ...
  const gameWalletSlice = beginCell()
    .storeAddress(wallet.address)
    .endCell()
    .asSlice();
  const { stack: gameWalletStack } = await client.runMethod(
    jettonMinterAddress,
    'get_wallet_address',
    [{ type: 'slice', cell: gameWalletSlice.asCell() }]
  );
...
export async function onRequestPost(context) {
  try {
    ...
    if (!userAddress || !payout || !ticketId) {
      return new Response(
        JSON.stringify({
          ...
          message: 'Ticket payload is missing required fields.',
        }),
        { status: 400 }
      );
    }
    ...
    if (e.message.includes('insufficient funds')) {
      // (KO) 실제 오류 메시지에 맞게 조정될 수 있음 (EN) May need adjustment for the actual error message
      return new Response(
        JSON.stringify({
          success: false,
          errorCode: 'INSUFFICIENT_FUNDS',
          message:
            'The game wallet has insufficient funds to pay out the prize.',
        }),
        { status: 503 }
      );
    }
