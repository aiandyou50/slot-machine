...
export async function onRequestPost(context) {
  try {
    ...
    if (!ownerAddress || !jettonMinterAddress) {
      return new Response(
        JSON.stringify({
          ...
          details: 'ownerAddress and jettonMinterAddress are required.',
        }),
        { status: 400 }
      );
    }
    ...
    const ownerWalletSlice = beginCell()
    ...
    return new Response(
      JSON.stringify({
        jettonWalletAddress: userJettonWalletAddress.toString(),
      }),
      {
    ...
    return new Response(
      JSON.stringify({ error: 'INTERNAL_SERVER_ERROR', details: e.message }),
      { status: 500 }
    );
  }
}
